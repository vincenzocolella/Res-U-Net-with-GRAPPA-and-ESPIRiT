...
class UnetModule(MriModule):
    ...
    def __init__(
        ...
        num_pool_layers=4,
        drop_prob=0.0,
        lr=0.001, #think about lowering it if the network isn't improving after a while
        lr_step_size=40,
        lr_gamma=0.1,
        weight_decay=0.0,
        **kwargs,
        ...
        self.ssim_loss = SSIMLoss(win_size = 7, k1 = 0.01, k2 = 0.03)
...
class UnetModule2(MriModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.unet = Unet_Mod(
            in_channels=1,
            out_channels=1,
         num_hidden_features=[32,64,128,256],n_resblocks=1,num_dilated_convs=1, dropout_min=0, dropout_max=0, gated=False, padding=1, kernel_size=3,group_norm=32)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {
            ...
            "output": output * std + mean,
            "target": batch.target * std + mean,
            "val_loss": F.l1_loss(output, batch.target) # SSIMLoss(output,batch.target)
        }
    '''
    def test_step(self, batch, batch_idx): #questo è il test step che parte
        ########à
